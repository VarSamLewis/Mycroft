...
def extract_graph(sql, database="default", schema="public", source_file=None):
    ...
    for sql_str in sql_strs:
        ...
        cte_map = _build_cte_map(ast)

        target_node = ast.find(exp.Create)
...
df = spark.sql("""
    CREATE TABLE users_enriched AS (
        SELECT
            ...
            e.event_count
        FROM users u
        JOIN event_counts e ON u.id = e.user_id
    )
""")

events = spark.read.table("raw_events")
events.write.saveAsTable("processed_events")
'''

    print("=== Python extraction ===")
    extracted = extract_sql_from_python(PYSPARK_CODE)
    pprint.pprint(extracted)

    print("\n=== Graph from Python ===")
    graph = extract_graph_from_python(
        PYSPARK_CODE, database="analytics", schema="public", source_file="jobs/etl.py"
    )
    try:
